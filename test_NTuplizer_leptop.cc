...
Leptop::analyze(const edm::Event& iEvent, const edm::EventSetup& pset) {
  ...
  if(pfjetAK8s.isValid()){
	...
	cout <<"size "<< (ak8jet.subjets(subjets)).size()<<endl;
	if((ak8jet.subjets(subjets)).size()==0)
	  nt1++;
	if((ak8jet.subjets(subjets)).size()==1)
	  nt2++;
...
{
  cout<<endl<<"No of event with 0 is "<<nt1;
  cout<<endl<<"No of event with 1 is "<<nt2;
  cout<<endl<<"No of event with 2 is "<<nt3;
}
