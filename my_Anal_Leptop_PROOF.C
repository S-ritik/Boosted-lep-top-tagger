...
void Anal_Leptop_PROOF::SlaveBegin(TTree * /*tree*/)
{
  ...
  for(int dr=0; dr<14; dr++){
   ...
   for(int i=0;i<15;i++) //new
    {
      char namein[1000];
      char titlein[1000];
      sprintf(namein,"%s",new_var_names[i]);
      sprintf(titlein,"%s",new_var_title[i]);
      hist_new_var[i] = new TH1D(namein,titlein,new_var_low[i],new_var_up[i],new_var_nbins[i]);
    } //new
...
Bool_t Anal_Leptop_PROOF::Process(Long64_t entry)
{
  ...
  if(isMC)
      ...
      for(int igen=0; igen<ngenparticles; igen++)
	{
	  ...
	  if(abs(genpartstatus[igen])==22 && abs(genpartpdg[igen])==6)
	    {
  ...
  if(isMC && isTT)
      ...
      for(int ilep=0; ilep<ndl; ilep++)
	{
	  for(int jlep=(ilep+1); jlep<ndl; jlep++)
	    {
	      if((abs(abs(genpartpdg[top_dlp[ilep]])-abs(genpartpdg[top_dlp[jlep]]))==1) && (genpartpdg[top_dlp[ilep]]*genpartpdg[top_dlp[jlep]]<0) && (genpartmompdg[top_dlp[ilep]]==genpartmompdg[top_dlp[jlep]]) && (genpartgrmompdg[top_dlp[ilep]]==genpartgrmompdg[top_dlp[jlep]]))
		{
  ...
  for(int mu=0; mu<nmuons; mu++)
      ...
      for(int kjet=0; kjet<npfjetAK4; kjet++)
	{
      ...
      if(nearjet>=0)
	{
	  TLorentzVector mu_mom; mu_mom.SetPtEtaPhiE(muonpt[mu],muoneta[mu],muonphi[mu],muonp[mu]);
  ...
  TLorentzVector pfAK8jet1, pfAK8jet2;
  float ptAK8max1= -10, ptAK8max2 = -100;
  int ijetptAK8max1 = -1, ijetptAK8max2 = -1;
  int fjet = 0;
  for(int ijet=0; ijet<npfjetAK8; ijet++)
    {
      
      if(!pfjetAK8jetID[ijet]) continue; //switch to tight ID (even tight ID calculation also do here in this code)
      ...
      if(isMC)
	{
      ...
      //new
      if( pfjetAK8pt[ijet] > ptAK8max1)
	{
	  ijetptAK8max1 = ijet;
	  ptAK8max1 = pfjetAK8pt[ijet];
	  pfAK8jet1.SetPtEtaPhiM( pfjetAK8pt[ijet], pfjetAK8eta[ijet], pfjetAK8phi[ijet], pfjetAK8mass[ijet]);
	}
      //new
  ...
  for(int ijet=0; ijet<npfjetAK4; ijet++){
    ...
    if(!pfjetAK4jetID[ijet]) continue; //switch to tight ID (even tight ID calculation also do here in this code)
    ...
    if (pfjetAK8muinsubeta[ijet] != -100 && pfjetAK8muinsubphi[ijet] != -100)
      {
	    ...
	    lep.SetPtEtaPhiM(pfjetAK8muinsubpt[ijet],pfjetAK8muinsubeta[ijet],pfjetAK8muinsubphi[ijet],pfjetAK8muinsubmass[ijet]);
  ...
  for(int imu=0; imu<nmuon1; imu++) {
    ...
    bool mu2_id = Muon_TightID(muonisGL[imu],muonisPF[imu],
                              muonchi[imu],muonhit[imu],muonmst[imu],
  ...
  for(int i=mucandv.size(); i<2; i++)
    {
      TLorentzVector vtemp;
      vtemp.SetPtEtaPhiE(-100.,-100.,-100.,-100.);
      mucandv.push_back(vtemp);
    }
    for(int i=elcandv.size(); i<2; i++)
    {
      TLorentzVector vtemp;
      vtemp.SetPtEtaPhiE(-100.,-100.,-100.,-100.);
      elcandv.push_back(vtemp);
    }
  
  if(mucandv[0].Pt()>elcandv[0].Pt())
    {
      lep1=mucandv[0];
      if(mucandv[1].Pt()>elcandv[0].Pt())
	lep2=mucandv[1];
      else
	lep2=elcandv[0];
    }
    ...
    {
      lep1=elcandv[0];
      if(elcandv[1].Pt()>mucandv[0].Pt())
	lep2=elcandv[1];
      else
	lep2=mucandv[0];
    }
  TLorentzVector bjetv;
  bjetv.SetPtEtaPhiE(-100.,-100.,-100.,-100.);
  float ptbmax = -100;
  for (int i=0;i<bjv.size();i++)
    {
      if(bjv[i].Pt()>ptbmax)
	bjetv=bjv[i];
    }
  ...
  if(PFMET>100.0)
    {
      hist_new_var[0]->Fill(PFMET,weight);
      hist_new_var[3]->Fill(PFMETPhi,weight);
    }
  if(lep1.Pt()>30)
    {
      hist_new_var[1]->Fill(lep1.Phi(),weight);
      hist_new_var[4]->Fill(lep1.Pt(),weight);
    }
  if(lep2.Pt()>30)
    {
      hist_new_var[2]->Fill(lep2.Phi(),weight);
      hist_new_var[5]->Fill(lep2.Pt(),weight);
    }
  if(pfAK8jet1.Pt() >400)
    {
      hist_new_var[6]->Fill(pfAK8jet1.Pt(),weight);
    }
  if(bjetv.Pt() >30)
    {
      hist_new_var[7]->Fill(bjetv.Pt(),weight);
    }
  if(PFMET>100.0)
    {
      if(lep1.Pt()>30)
	{
	  hist_new_var[8]->Fill( PhiInRange(PFMETPhi - lep1.Phi()) ,weight);
	  double mtlep1;
	  mtlep1=sqrt( 2.0*lep1.Pt()*PFMET*(1 - cos(PFMETPhi - lep1.Phi()) ) );
	  hist_new_var[12]->Fill(mtlep1,weight);
	}
      if(lep2.Pt()>30)
	{
	  hist_new_var[9]->Fill( PhiInRange(PFMETPhi - lep2.Phi()) ,weight);
	  double mtlep2;
	  mtlep2=sqrt( 2.0*lep2.Pt()*PFMET*(1 - cos(PFMETPhi - lep2.Phi()) ) );
	  hist_new_var[13]->Fill(mtlep2,weight);
	}
      if(pfAK8jet1.Pt()>400)
	{
	  hist_new_var[10]->Fill( PhiInRange(PFMETPhi - pfAK8jet1.Phi()) ,weight);
	  double rm;
	  rm=PFMET/pfAK8jet1.Pt();
	  hist_new_var[14]->Fill(rm,weight);
	}
      if(bjetv.Pt()>0) { hist_new_var[11]->Fill( PhiInRange(PFMETPhi - bjetv.Phi()) ,weight); }
    }
...
#ifdef E_MU_TTBar
  itrig_pass = (hlt_Mu37Ele27||hlt_Mu27Ele37);
  ...
  */
  ...
  if (hlt_Mu37Ele27 && !hlt_Mu27Ele37) {
    mulpt=40.0; ellpt=30.0;
    fsttrig=true;
  }
  else if (!hlt_Mu37Ele27 && hlt_Mu27Ele37) {
...
/*****/
  ...
  if (npfjetAK8>0) {
	...
	hist_dRdPhi[13]->Fill(PhiInRange(mucandv[itf].Phi() - elcandv[iuf].Phi()),weight); 
      ...
      for(int ijet=0; ijet<npfjetAK8; ijet++){      ///***
	if(pfjetAK8pt[ijet]<400. || pfjetAK8mass[ijet] < 0.0) { continue; }
	...
	//st_obs_mubtagwup[10]->Fill(pfjetAK8_bbyW_E[tmucand1],weight_btagwup);
	//st_obs_mubtagwup[11]->Fill(pfjetAK8_Kfactor[tmucand1],weight_btagwup);
	...
	//st_obs_mupuwdown[10]->Fill(pfjetAK8_bbyW_E[tmucand1],weight_puwdown);
	//st_obs_mupuwdown[11]->Fill(pfjetAK8_Kfactor[tmucand1],weight_puwdown);
	...
	//st_obs_mubtagwdown[10]->Fill(pfjetAK8_bbyW_E[tmucand1],weight_btagwdown);
	//st_obs_mubtagwdown[11]->Fill(pfjetAK8_Kfactor[tmucand1],weight_btagwdown);
	...
	//st_obsbtagwdown[10]->Fill(pfjetAK8_bbyW_E[telcand],weight_btagwdown);
	//st_obsbtagwdown[11]->Fill(pfjetAK8_Kfactor[telcand],weight_btagwdown);
